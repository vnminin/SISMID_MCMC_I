...
# The data points are time intervals (days) between the 
# detection of cases. There were 30 cases altogether, including
# the index case, so there are 29 time intervals.

# The observed data as time intervals (in days) between the removals
return(c(13,7,2,3,0,0,1,4,5,3,2,0,2,0,5,3,1,4,0,1,1,1,2,0,1,5,0,5,5))

}


##########################
# (2) Program removaltimes
##########################
removaltimes = function(intervals){
# July 16, 2012/KA
#
# This function outputs the removal times, based on 
# the observed time intervals between the removals. 

# N.B. The time origin (infection time of the index case) and 
#      the removal time of the index case are specific choices
#      pertaining to the Abakaliki smallpox data (cf. the intructions
#      to this exercise)
#
# INPUT intervals = time intervals between removals 

# Reserved space for removal times
n_intervals  = length(intervals)
remtimes     = rep(0,n_intervals+1)

# Model assumption: the removal in the index case occurred at day 14;
#                   the time origin will at day 0 = the day of infection
#                   of the index case
remtimes[1]   = 14

# Calculate the removal times for cases 2,...,30
remtimes[2:(n_intervals+1)] = remtimes[1] + cumsum(intervals)

# The output
return(remtimes) # all removal times 

}


############################
# (3) Program initializedata
############################
...
###################################
# (5) Program totaltime_infpressure
###################################
...
al = 1 # YOUR TASK IS TO FILL IN THE CORRECT 'ALPHA PARAMETER' 
...
par(mfrow=c(1,1))
hist(retain.sample$bet/retain.sample$gam,main="",xlab="reproduction number")
